...
async function validateHostedRunnerRequest(input = {}, options = {}) {
  ...
  if (tenantResolution.tenant_resolution_status === 'no_match') {
    ...
    if (missingCicdTeamCandidate) {
      errors.push(
        `The derived tenant CI/CD admin team '${missingCicdTeamCandidate.cicd_admin_team_name}' does not exist in organization '${request.organization}'. Provision the team before requesting tenant runners.`
      );
