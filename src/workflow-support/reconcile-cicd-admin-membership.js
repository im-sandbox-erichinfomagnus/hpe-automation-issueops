...
// Pure plan builder mirroring reconcile-team-members, extended with the
// ensure-team step: when the dedicated CI/CD admin team is missing it is
// created as a child of the tenant root team and every active root-team
// maintainer is assigned as its Team Maintainer.
function reconcileCicdAdminMembership(input = {}) {
  const request = input.request || {};
  const validatedPeople = input.validatedPeople || input.requested_people || [];
  const cicdAdminTeamExists = input.cicd_admin_team_exists === true;
  ...
  for (const person of validatedPeople) {
    const currentMember = cicdAdminTeamExists ? currentMemberMap.get(person.username) : null;
  ...
  const teamAction = cicdAdminTeamExists ? 'noop' : 'create_team';
  const maintainersToAssign = cicdAdminTeamExists ? [] : rootTeamMaintainers;
  const dryRun = Boolean(input.dry_run ?? request.dry_run);

  return {
    intake_mode: request.intake_mode || null,
    team_action: teamAction,
    cicd_admin_team_slug: normalizeLogin(input.cicd_admin_team_slug || request.cicd_admin_team_slug),
...
// Appends the dedicated cicd-admin team node to the canonical tenant registry
// record so the topology reflects the team created by this operation.
function persistCicdAdminTeamTopology(input = {}) {
