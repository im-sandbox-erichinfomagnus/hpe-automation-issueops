...
const { parseHostedRunnerDeletionRequest } = require('./parse-hosted-runner-deletion-request');
const {
...
async function validateHostedRunnerDeletionRequest(input = {}, options = {}) {
  const request = input.request_id ? input : parseHostedRunnerDeletionRequest(input);
  const errors = [];
  const warnings = [];
  ...
  const tenantResolution = await resolveTenantCicdContextFromRegistry(
    {
      ...
      operation: 'hosted_runner_deletion',
  ...
  if (tenantResolution.tenant_resolution_status === 'no_match') {
    ...
    if (missingCicdTeamCandidate) {
      errors.push(
        `The tenant topology admin team '${missingCicdTeamCandidate.cicd_admin_team_name}' does not exist in organization '${request.organization}'. Provision the team before managing tenant runners.`
      );
  ...
  if (
    ...
    if (existingRunner) {
      ...
      existingRunnerStatus = existingRunner.status || '';
    } else {
      warnings.push(`No hosted runner named '${request.runner_name_derived}' exists; execution will converge as no-op.`);
    }
  }
...
module.exports = {
  validateHostedRunnerDeletionRequest,
};
