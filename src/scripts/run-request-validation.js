...
async function runRequestValidation(options = {}) {
  ...
  const isTeamRepoAccess = !isRepoAdminMembership && isTeamRepoAccessParsedRequest(parsedRequest);
        const isTeamRepoAccessRemoval = !isRepoAdminMembership && isTeamRepoAccessRemovalParsedRequest(parsedRequest);
  const isTenantRepoCreation = !isRepoAdminMembership && isTenantRepoCreationParsedRequest(parsedRequest);
  ...
  const isTenantCreation = !isRepoAdminMembership && !isTenantRunnerOperation && !isTenantVariableManagement && !isRepositoryRulesetOperation && isTenantCreationParsedRequest(parsedRequest);
  const isTeamCreation = !isRepoAdminMembership && !isTenantVariableManagement && !isRepositoryRulesetOperation && isTeamCreationParsedRequest(parsedRequest);
  const isTeamHierarchy = !isRepoAdminMembership && !isTenantVariableManagement && !isRepositoryRulesetOperation && isTeamHierarchyParsedRequest(parsedRequest);
  ...
  try {
    ...
    if (operation === 'team_hierarchy' && tokenInfo.source !== 'ISSUEOPS_GITHUB_TOKEN') {
      ...
      } else if (isHostedRunnerCreation || isHostedRunnerDeletion || isHostedRunnerMove || isRunnerGroupCreation || isTenantVariableManagement || isRepoAdminMembership || isRepositoryRulesetOperation) {
        validation = {
          is_valid: false,
          request_status: 'validation_failed',
          errors: ['Workflow token secret is missing. Configure ISSUEOPS_GITHUB_TOKEN or GITHUB_TOKEN for validation.'],
