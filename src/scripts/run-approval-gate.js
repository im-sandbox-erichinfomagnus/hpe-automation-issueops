...
function buildAssignmentNote(operation) {
  ...
  if (operation === 'tenant_subteam_creation') {
    return 'Central issue assignment is for queue ownership only and does not authorize tenant subteam mutation.';
  }
...
async function runApprovalGate(options = {}) {
  ...
  if (
    (operation === 'team_membership' || operation === 'team_creation' || operation === 'team_hierarchy' || operation === 'team_repo_access' || operation === 'team_repo_access_removal' || operation === 'tenant_creation' || operation === 'tenant_repo_creation' || operation === 'tenant_subteam_creation') &&
          ...
          : auditArtifact.metadata && auditArtifact.metadata.operation === 'team_hierarchy'
            ...
            : auditArtifact.metadata && auditArtifact.metadata.operation === 'team_repo_access'
              ...
              : auditArtifact.metadata && auditArtifact.metadata.operation === 'tenant_creation'
                ? 'tenant_creation'
              : auditArtifact.metadata && ['hosted_runner_creation', 'hosted_runner_deletion', 'hosted_runner_move', 'runner_group_creation', 'tenant_variable_management', 'tenant_subteam_creation', 'repository_ruleset_creation', 'repository_ruleset_deletion'].includes(auditArtifact.metadata.operation)
  ...
  auditArtifact.execution.summary =
    auditArtifact.metadata && auditArtifact.metadata.operation === 'team_hierarchy'
      ...
      : auditArtifact.metadata && auditArtifact.metadata.operation === 'tenant_subteam_creation'
      ? auditArtifact.approval.approval_status === 'approved'
        ? 'Request approval was granted by the authorized designated target organization owner. No tenant subteam mutation was attempted in this phase.'
        : auditArtifact.approval.approval_status === 'not_requested'
          ? 'Request is still waiting for a requester-authored CSV attachment comment before approval can be evaluated. No tenant subteam mutation was attempted.'
        : auditArtifact.approval.approval_status === 'denied'
          ? 'Approval was denied because the approval comment did not come from the authorized designated target organization owner. No tenant subteam mutation was attempted.'
          : auditArtifact.approval.approval_status === 'invalidated'
            ? 'Approval was invalidated after the approval comment was removed. No tenant subteam mutation was attempted.'
            : 'Request is validated, centrally routed, and awaiting approval from the designated target organization owner. No tenant subteam mutation was attempted.'
