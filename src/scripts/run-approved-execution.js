...
const { assertRunnerGroupCreationAllowed: assertCicdAdminMembershipMutationAllowed } = require('../actions/runner-group-policy');
const { assertRunnerGroupCreationAllowed: assertRepoAdminMembershipMutationAllowed } = require('../actions/runner-group-policy');
  ...
  try {
    mutationDecision = isTenantCreation
      ...
      : isCicdAdminMembership
        ? assertCicdAdminMembershipMutationAllowed({
            approval_status: auditArtifact.approval.approval_status,
            approver_login: auditArtifact.approval.approver_login,
            designated_approver_login: auditArtifact.request.designated_approver_login,
            approver_role: auditArtifact.approval.approver_role,
      ...
      : isRepoAdminMembership
        ? assertRepoAdminMembershipMutationAllowed({
            approval_status: auditArtifact.approval.approval_status,
            approver_login: auditArtifact.approval.approver_login,
            designated_approver_login: auditArtifact.request.designated_approver_login,
            approver_role: auditArtifact.approval.approver_role,
        ...
        if (registryResult.status === 'blocked_missing_directory') {
          ...
          if (env.GITHUB_STEP_SUMMARY) {
            const summaryMsg = commitResult.status === 'committed'
              ? `âœ… **Registry Persistence**: Tenant registry committed to repository (${commitResult.commit_message})`
              : commitResult.status === 'noop'
                ? `â„¹ï¸ **Registry Persistence**: No registry changes to commit (file unchanged)`
                : `âš ï¸ **Registry Persistence**: Failed to commit registry record (${commitResult.message})`;
