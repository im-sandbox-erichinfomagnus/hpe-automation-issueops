...
function formatAuditSummary(auditArtifact = {}) {
  ...
  if (isHostedRunnerCreation || isHostedRunnerDeletion || isHostedRunnerMove) {
    return [
      ...
      `- Tenant matches: ${validation.tenant_resolution && validation.tenant_resolution.tenant_match_count || 0}`,
      `- Tenant parent team: ${request.tenant_team_slug || 'n/a'}`,
      `- Tenant CI/CD admin team: ${request.cicd_admin_team_slug || 'n/a'}${request.cicd_admin_team_matched_on ? ` (matched on ${request.cicd_admin_team_matched_on})` : ''}`,
  ...
  if (isTenantCreation) {
    return [
      ...
      `- Intended tenant admin: ${reconciliation.tenant_admin_intended_login || request.tenant_admin_login || 'n/a'}`,
      `- Tenant admin bootstrap action: ${reconciliation.tenant_admin_bootstrap_action || reconciliation.requester_bootstrap_action || 'n/a'}`,
      `- Requester bootstrap action: ${reconciliation.requester_bootstrap_action || 'n/a'}`,
      `- Creator maintainer behavior: ${reconciliation.creator_maintainer_behavior || 'github_auto_adds_creator_as_maintainer'}`,
      `- Requester maintainer normalization policy: ${reconciliation.requester_maintainer_normalization_policy || 'keep_maintainer'}`,
      `- Final maintainer list action taken: ${reconciliation.tenant_bootstrap_final_maintainer_action || 'n/a'}`,
