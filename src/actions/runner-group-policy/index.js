...
function assertRunnerGroupCreationAllowed(context = {}, options = {}) {
  ...
  if (approvalStatus !== 'approved') {
    throw new Error(`Runner group creation blocked because approval status is ${approvalStatus}`);
  }

  if (!isEligibleApproverRole(approverRole, options) || approverAuthorizationState !== 'authorized') {
    throw new Error(`Runner group creation blocked because approver role ${approverRole} is not eligible`);
  }

  if (!tokenInfo || !tokenInfo.token) {
    throw new Error('Runner group creation blocked because no workflow token is available');
  }

  if (!tokenInfo.is_pat_backed || !tokenInfo.supports_org_mutation) {
    throw new Error('Runner group creation blocked because the workflow token is not PAT-backed for organization mutation');
  }
