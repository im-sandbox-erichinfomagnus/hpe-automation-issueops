...
function assertRepositoryCreationAllowed(context = {}, options = {}) {
  ...
  if (approvalStatus !== 'approved') {
    throw new Error(`Repository creation blocked because approval status is ${approvalStatus}`);
  }

  if (!isEligibleApproverRole(approverRole, options) || approverAuthorizationState !== 'authorized') {
    throw new Error(`Repository creation blocked because approver role ${approverRole} is not eligible`);
  }

  if (!tokenInfo || !tokenInfo.token) {
    throw new Error('Repository creation blocked because no workflow token is available');
  }

  if (!tokenInfo.is_pat_backed || !tokenInfo.supports_team_repo_access_mutation) {
    throw new Error('Repository creation blocked because the workflow token is not PAT-backed for repository mutation');
  }
