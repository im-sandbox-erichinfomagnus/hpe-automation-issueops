...
Create one tenant-prefixed GitHub-hosted runner in a target organization through an approval-gated, auditable IssueOps request authorized by tenant topology admin membership.
...
3. The `create-tenant-hosted-runner` workflow validates the request (tenant resolution, topology admin membership, name derivation, runner group resolution) and publishes an audit artifact plus step summary.
...
**Preconditions**: Tenant registry record present; topology admin team exists with requester as active member; no runner with the derived name exists.
...
### Scenario 2: Requester not a topology admin member

**Preconditions**: Requester is not an active member of `<tenant-slug>-admin`.
...
### Scenario 3: topology admin team missing

**Preconditions**: The canonical tenant topology admin team (`<tenant-slug>-admin`) does not exist.
...
**Preconditions**: Request approved, then requester removed from the topology admin team before execution.
...
|---|---|---|
| Validation error: no authorized tenant context | Tenant name does not match a registry record, or requester lacks topology admin membership | Verify `tenant_display_name` in the registry record and the requester's active membership on `<tenant-slug>-admin` |
| Validation error: topology admin team missing | `<tenant-slug>-admin` not provisioned | Provision the team through the tenant topology admin governance process, then re-run |
