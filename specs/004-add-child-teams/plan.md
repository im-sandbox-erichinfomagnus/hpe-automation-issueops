# Implementation Plan: Add Child Teams Workflow

**Branch**: `004-add-child-teams` | **Date**: 2026-05-15 | **Spec**: [spec.md](./spec.md)
**Input**: Feature specification from `/specs/004-add-child-teams/spec.md`
...
repository to attach one or more existing child teams under one existing parent
team in a target GitHub organization, validates current hierarchy state and
approver eligibility, enforces central approval by one shared hierarchy
approver, and applies only missing parent-child links. The implementation will
use YAML issue forms, `issue-ops/parser`, thin GitHub Actions workflow shims,
shared Node-based workflow support modules under `src`, a PAT-backed workflow
credential stored as `ISSUEOPS_GITHUB_TOKEN`, structured audit artifacts, and
reconciliation-first execution with dry-run and bounded retry behavior.
...
**Primary Dependencies**: `issue-ops/parser@v5`, GitHub REST team endpoints from Node-based workflow support modules, `actions/upload-artifact`, `actionlint`  
**Authentication Model**: A PAT-backed workflow token loaded from the `ISSUEOPS_GITHUB_TOKEN` secret is used for central-repository issue updates, target organization validation, approver verification, hierarchy reads, and team hierarchy mutation  
...
**Testing**: `actionlint`, parser fixture tests, authorization and approval tests, hierarchy reconciliation tests, integration dry-run tests, and replay tests for idempotent no-op reruns  
...
**Constraints**: least privilege within PAT scope, approval before mutation, hierarchy-only workflow scope, idempotent link behavior, no team creation or membership mutation in this feature, reject re-parenting in v1, reject cycle-creating requests, REST API rate-limit backoff, no repo-hosted source of truth  
**Scale/Scope**: Centralized enterprise IssueOps workflow for repeated team-hierarchy requests across multiple target organizations, starting with additive child-team linking only
...
```text
specs/004-add-child-teams/
├── plan.md              # This file (/speckit.plan command output)
├── research.md          # Phase 0 output (/speckit.plan command)
├── data-model.md        # Phase 1 output (/speckit.plan command)
├── quickstart.md        # Phase 1 output (/speckit.plan command)
├── contracts/
│   └── add-child-teams-workflow.yaml
...
constitution without exception. Keep `.github/ISSUE_TEMPLATE/add-child-teams.yml`
as the intake surface, keep `.github/workflows/add-child-teams.yml` as the thin
GitHub-required shim, and place validation, approval, reconciliation, PAT
loading, hierarchy API helpers, and audit generation under `src/`. Testing stays
