...
const { assertCostCenterMutationAllowed } = require('../../src/actions/cost-center-policy');
...
test('manage-cost-centers workflow wires validation, approval, and execution scripts', () => {
  ...
  assert.match(workflow, /run-cost-center-validation\.js/);
  assert.match(workflow, /run-cost-center-approval\.js/);
  assert.match(workflow, /run-cost-center-execution\.js/);
